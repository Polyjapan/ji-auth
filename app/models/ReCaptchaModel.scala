...
/**
  * @author zyuiop
  */
class ReCaptchaModel @Inject()(ws: WSClient)(implicit ec: ExecutionContext) {
  def checkCaptcha(app: data.App, response: String): Future[ReCaptchaResponse] = app.recaptchaPrivate match {
    case Some(key) => doCheckCaptcha(key, response)
    case _ => Future(ReCaptchaResponse(true, Some(new DateTime(System.currentTimeMillis())), Some("nocaptcha.void")))
  }

  def doCheckCaptcha(secretKey: String, response: String): Future[ReCaptchaResponse] = {
  ...
  def checkCaptchaWithExpiration(app: data.App, response: String, hours: Int = 6): Future[ReCaptchaResponse] = app.recaptchaPrivate match {
    case Some(key) => doCheckCaptchaWithExpiration(key, response, hours)
    case _ => Future(ReCaptchaResponse(true, Some(new DateTime(System.currentTimeMillis())), Some("nocaptcha.void")))
  }

  def doCheckCaptchaWithExpiration(secretKey: String, response: String, hours: Int = 6): Future[ReCaptchaResponse] = {
